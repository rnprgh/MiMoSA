...
- `src/MiMoSA/`: package source.
- `examples/MiMoSA_Sample_Workflow/`: executed example notebook, licensed
  sample video, ND2-derived frames, and calibration metadata.
- `examples/MiMoSA_Comparative_Stats/`: portable, unexecuted multi-run
  comparison notebook; source run exports are not included.
- `docs/RABITPY_LINEAGE.md`: relationship to RABiTPy and namespace migration.
- `docs/TRACKER_COMPARISON.md`: basic-versus-advanced tracker comparison.
- `docs/MODULE_IMPROVEMENTS.md`: module-level quality-of-life improvements.
- `docs/STATS_EXPORTED_DATA_FILES_SUMMARY.md`: statistics export guide.
- `docs/SAMPLE_MEDIA_PROVENANCE.md`: sample-media provenance, technical
  details, licensing, and upstream-video exclusion record.
- `licenses/`: exact upstream license artifact retained for audit.
- `tools/fiji/`: ND2 metadata export helper and instructions.
...
The executed example is
`examples/MiMoSA_Sample_Workflow/01_MiMoSA_Sample_Walkthrough.ipynb`.
...
The notebook retains its executed outputs so readers can inspect the completed
workflow. Those retained outputs report an 851-frame source run; the bundled
inputs are a 100-frame publication excerpt, so rerunning the notebook on the
distributed inputs will not reproduce the same observation counts. External
files under `02_Outputs/` are ignored because they can be regenerated by
...
DataFrame, CSV, Excel, and NumPy export schemas remain compatible with the
documented workflow. Python object pickles that encode the old
`RABiTPy.<module>` namespace are not guaranteed to load as MiMoSA objects; use
the tabular and metadata exports for migration. See
